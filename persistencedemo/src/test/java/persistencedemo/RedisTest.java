...
import persistencedemo.biz.redis.JedisPubSubListener;
...
public class RedisTest {
    ...
    /**
     * 批量操作key keySlot算法中，如果key包含{}，就会使用第一个{}内部的字符串作为hash key，这样就可以保证拥有同样{}内部字符串的key就会拥有相同slot。 参考
     * http://brandnewuser.iteye.com/blog/2314280
    ...
    public void msetTest() throws InterruptedException {
         ...
         * 直接这样写，会报错：redis.clients.jedis.exceptions.JedisClusterException: No way to dispatch this
         * command to Redis Cluster because keys have different slots. 这是因为key不在同一个slot中
         */
    ...
    /**
     * MSETNX 命令：它只会在所有给定 key 都不存在的情况下进行设置操作。 http://doc.redisfans.com/string/mset.html
     */
    @Test
    public void msetnxTest() throws InterruptedException {
        Long msetnx = jedisCluster.msetnx("{" + prefix + KEY_SPLIT + "}" + "name", "张三",
                "{" + prefix + KEY_SPLIT + "}" + "age", "23", "{" + prefix + KEY_SPLIT + "}" + "address", "adfsa",
                "{" + prefix + KEY_SPLIT + "}" + "score", "100");
        System.out.println(msetnx);

        System.out.println(
                jedisCluster.mget("{" + prefix + KEY_SPLIT + "}" + "name", "{" + prefix + KEY_SPLIT + "}" + "age",
                        "{" + prefix + KEY_SPLIT + "}" + "address", "{" + prefix + KEY_SPLIT + "}" + "score"));// [张三,
                                                                                                               // 23,
                                                                                                               // adfsa,
                                                                                                               // 100]

        // name这个key已经存在，由于mset是原子的，该条指令将全部失败
        msetnx = jedisCluster.msetnx("{" + prefix + KEY_SPLIT + "}" + "phone", "110",
                "{" + prefix + KEY_SPLIT + "}" + "name", "张三", "{" + prefix + KEY_SPLIT + "}" + "abc", "asdfasfdsa");
        System.out.println(msetnx);


        System.out.println(
                jedisCluster.mget("{" + prefix + KEY_SPLIT + "}" + "name", "{" + prefix + KEY_SPLIT + "}" + "age",
                        "{" + prefix + KEY_SPLIT + "}" + "address", "{" + prefix + KEY_SPLIT + "}" + "score",
                        "{" + prefix + KEY_SPLIT + "}" + "phone", "{" + prefix + KEY_SPLIT + "}" + "abc"));// [张三,
                                                                                                           // 23,
                                                                                                           // adfsa,
                                                                                                           // 100,
                                                                                                           // null,
    ...
    public void setTest() throws InterruptedException {
        ...
        System.out.println("c是否在集合A中：" + jedisCluster.sismember(keyA, "c"));// 判断 member 元素是否集合 key
                                                                            // 的成员。
        /*
         * 从 Redis 2.6 版本开始， SRANDMEMBER 命令接受可选的 count 参数： 如果 count 为正数，且小于集合基数，那么命令返回一个包含 count
        ...
        /*
         * SMOVE 是原子性操作。 如果 source 集合不存在或不包含指定的 member 元素，则 SMOVE 命令不执行任何操作，仅返回 0 。否则， member 元素从
         * source 集合中被移除，并添加到 destination 集合中去。 当 destination 集合已经包含 member 元素时， SMOVE 命令只是简单地将
         * source 集合中的 member 元素删除。 当 source 或 destination 不是集合类型时，返回一个错误。
         * 注：不可以在redis-cluster中使用SMOVE：redis.clients.jedis.exceptions.JedisClusterException: No way
         * to dispatch this command to Redis Cluster because keys have different slots.
    ...
    public void sortedSetTest() throws InterruptedException {
        ...
        System.out.println("zrange: " + jedisCluster.zrange(keyA, 0, -1));// 返回有序集 key
                                                                          // 中，指定区间内的成员。按score从小到大
        System.out.println("zrevrange: " + jedisCluster.zrevrange(keyA, 0, -1));// 返回有序集 key
        ...
        System.out.println("zrank: " + jedisCluster.zrank(keyA, "c"));// 返回有序集 key 中成员 member
                                                                      // 的排名。其中有序集成员按 score
                                                                      // 值递增(从小到大)顺序排列。
        System.out.println("zrevrank: " + jedisCluster.zrevrank(keyA, "c"));// 返回有序集 key 中成员 member
                                                                            // 的排名。其中有序集成员按 score
                                                                            // 值递增(从大到小)顺序排列。

        System.out.println("zrem: " + jedisCluster.zrem(keyA, "c", "a"));// 移除有序集 key
         ...
         * SUM|MIN|MAX] 计算给定的一个或多个有序集的并集，其中给定 key 的数量必须以 numkeys 参数指定，并将该并集(结果集)储存到 destination 。
         * 默认情况下，结果集中某个成员的 score 值是所有给定集下该成员 score 值之 和 。 WEIGHTS 使用 WEIGHTS 选项，你可以为 每个 给定有序集 分别
         * 指定一个乘法因子(multiplication factor)，每个给定有序集的所有成员的 score 值在传递给聚合函数(aggregation
         * function)之前都要先乘以该有序集的因子。 如果没有指定 WEIGHTS 选项，乘法因子默认设置为 1 。 AGGREGATE 使用 AGGREGATE
         * 选项，你可以指定并集的结果集的聚合方式。 默认使用的参数 SUM ，可以将所有集合中某个成员的 score 值之 和 作为结果集中该成员的 score 值；使用参数 MIN
         * ，可以将所有集合中某个成员的 最小 score 值作为结果集中该成员的 score 值；而参数 MAX 则是将所有集合中某个成员的 最大 score 值作为结果集中该成员的
         * score 值。
         */
    ...
    public void script() throws InterruptedException {
        ...
        /*
         * 其中 "return {KEYS[1],KEYS[2],ARGV[1],ARGV[2]}" 是被求值的 Lua 脚本，数字 2 指定了键名参数的数量， key1 和 key2
         * 是键名参数，分别使用 KEYS[1] 和 KEYS[2] 访问，而最后的 first 和 second 则是附加参数，可以通过 ARGV[1] 和 ARGV[2] 访问它们。
         *
    ...
    /**
     * redis中的lua脚本做了很多限制，防止随机性的发生。比如lua脚本中返回的总是有序的集合。 详情见 http://doc.redisfans.com/script/eval.html
     * - 纯函数脚本
     */
    @Test
    ...
    /**
     * 模拟先进先出 定长 队列 参考 http://www.cnblogs.com/stephen-liu74/archive/2012/02/14/2351859.html
